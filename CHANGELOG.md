...
- **Dependabot reverted from the `bun` ecosystem back to `npm`.** The 09-04 Bun migration pointed
  Dependabot at the `bun` ecosystem, reasoning that the npm ecosystem edits `package.json` only and
  leaves `bun.lock` stale. That reasoning was right about the symptom and **wrong about the fix**:
  Dependabot's bun parser supports only `lockfileVersion 1` and Bun 1.4 writes 2, so the very next
  run failed with `Unsupported bun.lock 'lockfileVersion' 2 in /bun.lock`.
  This fleet had already diagnosed and fixed the identical thing twice before this repo hit it
  (`deepthinking-mcp` 524ada7f, `fzf-mcp` 387ee494; and again in `Mathts` c0dd0d1).
  **It hides well, which is why it earns an entry rather than a quiet revert:** security ALERTS are
  dependency-graph based and keep working, so nothing goes red. What dies is automated
  REMEDIATION — and "0 open PRs" then reads as health when it partly means Dependabot cannot open
  one. `bun.lock` stays authoritative and is regenerated by `bun install`.
